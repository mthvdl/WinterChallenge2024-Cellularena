...
Credentials are read from  pz_cellularena/.env
...
from project_paths import ensure_dir, shared_replays_dir
from replay_transform import codingame_to_core_raw
...
def save_core_replay(game_id: int, data: Dict) -> Path:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Download CodingGame Cellularena replays")
    ...
    parser.add_argument("--game", type=str, default="cellularena", help="Game namespace for shared replay data")
    ...
    if tv:
        tv_id, tv_data = tv
        core_path = save_core_replay(tv_id, tv_data)
    ...
    if args.game_id:
        ...
        if data:
            core_path = save_core_replay(args.game_id, data)
