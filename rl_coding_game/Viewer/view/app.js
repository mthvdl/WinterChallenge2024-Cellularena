...
let frameIndex = 0
let frameProgress = 1
let frameElapsedMs = FRAME_DURATION
let paused = true
let speed = 1
...
function updateScene() {
  ...
  window.dispatchEvent(new CustomEvent('acTurnChanged', { detail: { frameIndex, total: states.length } }))
}
...
async function loadReplayObject(rawReplay) {
  ...
  // expose for action console
  window._acFrames = rawReplay.frames || []
  window._acAgents = rawReplay.agents || []
...
app.ticker.add((delta) => {
  if (viewModule) {
    if (!paused && states.length > 1) {
      frameElapsedMs += app.ticker.deltaMS * speed
      while (frameElapsedMs >= FRAME_DURATION && frameIndex < states.length - 1) {
        frameElapsedMs -= FRAME_DURATION
        frameIndex += 1
        turnRange.value = String(frameIndex)
      }
      frameProgress = Math.min(1, frameElapsedMs / FRAME_DURATION)
      if (frameIndex === states.length - 1 && frameProgress >= 1) {
        frameProgress = 1
        frameElapsedMs = FRAME_DURATION
        paused = true
...
playPauseBtn.addEventListener('click', () => {
  ...
  paused = !paused
  if (!paused && frameIndex === states.length - 1 && frameProgress >= 1) {
    frameIndex = 0
    frameElapsedMs = 0
    frameProgress = 0
    turnRange.value = '0'
  }
  if (!paused && frameProgress >= 1 && frameIndex < states.length - 1) {
    frameElapsedMs = 0
    frameProgress = 0
  }
  playPauseBtn.textContent = paused ? 'Play' : 'Pause'
