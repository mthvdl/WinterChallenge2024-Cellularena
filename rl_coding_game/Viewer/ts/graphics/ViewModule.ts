...
export class ViewModule {
  ...
  handleFrameData (frameInfo: FrameInfo, raw: string): FrameData {
    ...
    // CodingGame frame events can use multi-phase timings where event end values
    // exceed frameDuration (e.g. 1400, 2800, 3200). Normalize using the frame's
    // max end so all event windows map to [0, 1] for this frame.
