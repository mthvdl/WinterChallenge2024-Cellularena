...
For the generic framework commands (training, downloading, Azure), see `rl_coding_game/AGENTS.md`.
...
Fix failures by editing `games/cellularena/game/game.py`, then re-run.
...
# Open: http://localhost:8000/games/cellularena/viewer/view/index.html
...
| `games/cellularena/viewer/` | Standalone TypeScript replay visualizer |
...
| `viewer_server.py` | Runtime conversion server (load raw replay via HTTP) |
...
| Build viewer | `cd games/cellularena/viewer && npm install && npm run build` |
...
| Train (cellularena) | `python train_rainbow.py --env-factory games.cellularena.factories:make_env --game cellularena --experiment-name exp_001 --total-steps 500000 --n-envs 4 --self-play --reset-replay` |
