...
_MIRROR_DIR = np.asarray((2, 3, 0, 1), dtype=np.int64)
...
def encode_observation(observation: dict[str, Any]) -> np.ndarray:
    ...
    storage = np.asarray(observation.get("storage", np.zeros((2, 4))), dtype=np.float32)
    self_index = int(np.asarray(observation.get("self_player_idx", 0)).reshape(-1)[0])
    ...
    scaled_storage = np.clip(storage * 50.0, 0.0, 99.0)
    ...
    features[:, :, 92] = float(np.asarray(observation.get("turn", 0)).reshape(-1)[0]) * 100.0
