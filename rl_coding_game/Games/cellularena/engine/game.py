...
class Game:
	...
	def reset(self) -> None:
		...
		for player in self.players:
			if player.idx == 0:
				...
				# Vertical-axis mirror matches grid_maker symmetry
				pos = Coord(self.grid.width - 1 - spawn.x, spawn.y)
	...
	def step(self, actions: Dict[int, List[int]]) -> Tuple[bool, Dict[int, float]]:
		...
		# No intermediate rewards - only sparse win/loss/tie at the end
