...
from Games.cellularena.engine.obs import TemporalObservationBuilder
from Games.cellularena.engine.coord import Direction
from Games.cellularena.engine.game import ACTIONS_PER_ORG, MAX_ROOTS, Game, GrowthCommand


def create_adapter(
	include_player1: bool = True,
	obs_history_steps: int = 1,
) -> ReplayTransitionAdapter:
	return CellularenaCoreReplayAdapter(
		include_player1=include_player1,
...
class CellularenaCoreReplayAdapter(ReplayTransitionAdapter):
	...
	def iter_transitions(self, replay_path: Path) -> Iterable[Transition]:
		...
		for turn_entry in data.get("turns", []):
			...
			obs_p0 = self._obs_builder.transform(0, game.get_observation(0))
			obs_p1 = self._obs_builder.transform(1, game.get_observation(1))
			...
			next_obs_p0 = self._obs_builder.transform(0, game.get_observation(0))
			next_obs_p1 = self._obs_builder.transform(1, game.get_observation(1))
	...
	def _resolve_initial_storage(self, data: dict, replay_path: Path) -> List[List[int]]:
		...
		if game_id is not None:
			...
			if codingame_path.exists():
				try:
					from Games.cellularena.engine.tools.replay_transform import _infer_initial_storage, load_replay_from_dict
